...
def myntra(name):
    try:
        global myntra
        name1 = name.replace(" ","+")
        myntra=f'https://www.myntra.com/sch/i.html?_from=R40&_trksid=m570.l1313&_nkw={name1}&_sacat=0'
        res = requests.get(f'https://www.myntra.com/sch/i.html?_from=R40&_trksid=m570.l1313&_nkw={name1}&_sacat=0',headers=headers)

        soup = BeautifulSoup(res.text,'html.parser')
        length = soup.select('.s-item__price')
        myntra_page_length=int(len(length))
        for i in range (0,myntra_page_length):
            info = soup.select('.SECONDARY_INFO')[i].getText().strip()
            info = info.upper()
            if info=='BRAND NEW':
                myntra_name = soup.select('.s-item__title')[i].getText().strip()
                name=name.upper()
                myntra_name=myntra_name.upper()
                if name in myntra_name[:25]:
                    myntra_price = soup.select('.s-item__price')[i].getText().strip()
                    myntra_name = soup.select('.s-item__title')[i].getText().strip()

                    myntra_price = myntra_price.replace("INR","₹")

                    myntra_price=myntra_price[0:14]
                    break
                    return f"{myntra_name}\nPrise : {myntra_price}\n"

                else:
                    i+=1
                    i=int(i)
                    if i==myntra_page_length:

                        myntra_price = '           Product Not Found'
                        break
...
l2 =  Label(window, text="myntra", font=("courier", 20))
